...
export default class FacilityButtonTrack extends Component {
        ...
        if (this.state.sport === 'Basketball'){
            ...
            this.populateSubGymArr(
                <TouchableOpacity style={styles.sub_sub_gym} key={1} onPress={() => this.toggleRatingViewVisibility()}>
                    <ImageBackground style={styles.image} source={require('./Icons/tracks.png')}>
    ...
    addData(){
        console.log("BABABANA");
        fetch('https://calendar.google.com/calendar/embed?src=brown.edu_1gklqf5558hsd1le5io9m7mnak%40group.calendar.google.com&ctz=America%2FNew_York').then((response) => console.log("AMCIK EREN", response.json()));
    }
        ...
        if (this.state.isRatingViewVisible){
            ...
            return (
                <View style={styles.rating_view}>
                    ...
                    <Button title="Submit" onPress={console.log("annen")}></Button>
                </View>
            );
        } else {
	    ...
	    return(
            <View>
    	    	<View style={styles.container}>
    		       	<TouchableOpacity style={styles.sub_gym} onPress={() => this.addData()}>
